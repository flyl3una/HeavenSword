...
class Domain(models.Model):
    ...
    level = models.IntegerField()       #二级，三级，四级域名，
...
class Finger(models.Model):
    # id = models.IntegerField(max_length=32, auto_created=1, primary_key=True)
    single_task_id = models.ForeignKey(SingleTask)
    batch_task_id = models.ForeignKey(BatchTask)
...
class DomainBrute(models.Model):
    ...
    domain_brute_result_json = models.CharField(max_length=512)     #后期可将所有子域名分别存为domain表中
...
class Spider(models.Model):
    ...
    # spider_result_json = models.TextField(max_length=102400)      #改为分别存放url
...
class ExploitAttack(models.Model):
    ...
    exploit_attack_result_json = models.CharField(max_length=256)   #后期可改为每个漏洞分表